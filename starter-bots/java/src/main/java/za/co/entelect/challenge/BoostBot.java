...
/**
 * Represents a bot that implements the Boost Strategy:
 * Get as much boost as possible, hoping to outspeed the opponent
 */
public class BoostBot {
    ...
    public BoostBot(GameState gameState) {
    ...
    public Command run() {
        List<Object> blocks = getBlocksInFront(this.myCar.position.lane, this.myCar.position.block);
        ...
        if (blocks.contains(Terrain.MUD) || blocks.contains(Terrain.OIL_SPILL) || blocks.contains(Terrain.WALL)) {
            ...
            if (this.canTurn.get(0)) {
                leftBlocks = getBlocksInFront(this.myCar.position.lane - 1, this.myCar.position.block);
            }
            if (this.canTurn.get(1)) {
                rightBlocks = getBlocksInFront(this.myCar.position.lane + 1, this.myCar.position.block);
            }

            if (this.canTurn.get(0) && this.canTurn.get(1)) {
                if (leftBlocks.contains(Terrain.BOOST)) {
                    return TURN_LEFT;
                } else if (rightBlocks.contains(Terrain.BOOST)) {
                    return TURN_RIGHT;
                } else {
        ...
        // Other power-up logic
        if (this.myCar.speed == BoostBot.maxSpeed) {
            if (hasPowerUp(PowerUps.OIL, this.myCar.powerups)) {
                return OIL;
            } else if (hasPowerUp(PowerUps.EMP, this.myCar.powerups)) {
                return EMP;
            }
        }
    ...
    private List<Object> getBlocksInFront(int lane, int block) {
        ...
        for (int i = max(block - startBlock, 0); i <= block - startBlock + BoostBot.maxSpeed; i++) {
